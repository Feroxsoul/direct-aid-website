-- Restore SECURITY DEFINER for helper functions used by RLS/policies.
-- After the SECURITY INVOKER change (advisor follow-up), admin login started redirecting
-- back to /admin/login?error=unauthorized even when the admin_users row exists.
-- SECURITY DEFINER matches the original working setup and avoids RLS/policy evaluation issues.
